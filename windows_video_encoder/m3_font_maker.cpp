...
float sRGBtoLin(float colorChannel) {
	...
	if (conversion_mode == 1)
	{
		...
		return sc * pow(((colorChannel*1.2)), 2.4);
	}
...
#define _CRT_SECURE_NO_WARNINGS
int main_encoder(char *name);
#define FRAMES_UNTIL_NEXT_VQ 200
/******************************************************************************
...
char dithered_tile[8 * 8];
void dither_tile_o(float *tile_in)
{
	int error = 0, x, y;
	int dir = 0;
	float e = 0;
...
void dither_tile_sierra_lite(float *tile_in)
{
	int error = 0, x, y;
	int dir = 0;
	float e = 0;
...
void dither_tile_sierra_lite_b(float *tile_in)
{
	int error = 0, x, y;
	int dir = 0;
	float e = 0;
...
void dither_tile(float *tile_in)
{
	int error = 0, x, y;
	int dir = 0;
	float e = 0;
...
void store_frame(unsigned char *frame, int xres, int yres)
{
	...
	for (y = 0; y < 64; y++)
		for (x = 0; x < 128; x++)
		{
			...
			float c = frame[x + y * 128];// sRGBtoLin(frame[x + y * 128] / 255.0)*255.0;
...
void main(int argc, char* argv[])
{
	...
	for (i=0;;i+=2)
	{
		...
		img = new CImage();
		char tmp[128];
		sprintf(tmp, "/dev/rick%04d.png", i);
		printf("%s\r", tmp);
		HRESULT res=img->Load(tmp);
		if (res != S_OK)break;
		conv();
		display_img(im, 128, 64,512,256,1);
		delete img;

		store_frame(im, 128, 64);
		printf(".");
	}
	*/
	...
	if ((argc == 1)||(argc>3))
	{
		printf("usage: %s file.mp4/avi [conversion mode:0-2]\r\n");
		exit(0);
	}
	if (argc == 3)conversion_mode = atoi(argv[2]);
	
	read_video((char *)argv[1]);
	main_encoder((char*)"encoded_video");
...
//		printf("frame start %d, f_end %d", frame_start, frame_end);
		...
		for (frc = 0; frc < frames_chunk; frc++)
		{
			...
			if (frc == 100)	display_img(c_frame, 128, 64, 512, 128, 1);
			...
			if (frc%frames_chunk == 100)	display_img(c_frame2, 256, 64, 512 + 256, 128, 1);

			//			display_img(buf_out2, 128, 64, 512, 0, 1);
		}
		if (dump)
		{
		...
		for (i = 0; i < vq_nr; i++)
		{
			if ((!unused) && (v[i].usage < 8))//&&(!(rnd()&0xff)))
			{
				ccc--;
				if (ccc > 0)
					c = 25;
...
//	void Compress_Chunk(unsigned char *videodata, int frames_chunk, int nr_chunk)
	...
	int pos;
	int cnt = 0;
	for (pos = 0; pos < frames_stored;pos+=200)
	{
		int frames_left = frames_stored - pos;
		int chunk_len = frames_left;
		if (chunk_len > 200)chunk_len = 200;
...
int main_encoder(char *fname)
{
	...
	int chunk = 0;
	int frames_chunk = FRAMES_UNTIL_NEXT_VQ;
